...
def load_index(path):
    ...
    inverted_index = {}
    with open(path + '/inverted_index/' + index.app.config["INDEX_PATH"],
              'r', encoding="utf-8") as file_ii:
        for line in file_ii:
            term, idf, *docs_list = line.strip().split()
            inverted_index[term] = [float(idf), {}]
            for i in range(0, len(docs_list), 3):
                doc_id, term_freq, norm = docs_list[i:i+3]
                inverted_index[term][1][int(doc_id)] = [float(term_freq),
                                                        float(norm)]

    # stopwords = set(stopword)
    with open(path + '/stopwords.txt', 'r', encoding="utf-8") as file_sw:
        stopwords = set(stopword.strip() for stopword in file_sw)

    # pagerank = { doc_id : rank }
    pagerank = {}
    with open(path + '/pagerank.out', 'r', encoding="utf-8") as file_pr:
        for line in file_pr:
